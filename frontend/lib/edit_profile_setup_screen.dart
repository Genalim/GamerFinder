...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  String _activeField = "";
  bool _nickValid = true, // Одразу валідні, бо вже існують
      _emailValid = true,
      _passValid = true;
  ...
  String _nickMsg = "";
  Color _nickColor = const Color(0xFF00F5A0);
  String _emailMsg = "";
  Color _emailColor = const Color(0xFF00F5A0);
  String _passMsg = "Leave blank to keep current password";
  ...
  // Метод застосування змін (Зберегти профіль)
  Future<void> _applyChanges() async {
    ...
    try {
      ...
      // Формуємо мапу підключених акаунтів
      Map<String, String> connectedAccounts = {};
      _platformControllers.forEach((key, controller) {
        if (controller.text.isNotEmpty) {
          connectedAccounts[key] = controller.text;
        }
      });

      // Відправляємо PUT/PATCH запит на оновлення профілю
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) Navigator.pop(context); // Повертаємось до налаштувань
      } else {
  ...
  Future<void> _checkNicknameAvailability(String nickname) async {
    // Якщо нікнейм не змінився - перевіряти не треба
    if (nickname == UserSession().currentUser?.nickname) {
      setState(() {
        _nickMsg = "";
    ...
    try {
      ...
      debugPrint("Помилка при перевірці: $e");
    }
  }
  ...
  bool get _isFormReady => _nickValid && _emailValid;
  ...
  Future<void> _validateNick(String v) async {
    ...
    setState(() { _nickValid = false; });

    if (v.isEmpty) {
      setState(() {
        _nickMsg = "Nickname cannot be empty";
        _nickColor = const Color(0xFFFF3B5C);
      });
      return;
    }
  ...
  void _validatePass(String v) {
    setState(() {
      if (v.isEmpty) {
        _passValid = true; // Можна залишити порожнім, якщо не міняємо
        _passMsg = "Leave blank to keep current password";
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) {
        if (didPop) {
          // Нічого глобального не зберігаємо в менеджер сетапу, все йде напряму на сервер при Apply
        }
      },
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                            ...
                            _buildMainField(_nicknameController, 'Change your nickname', 'nick', _nickMsg, _nickColor, _nickValid, _validateNick, 145),
                            const SizedBox(height: 10),
                            _buildMainField(_emailController, 'Change your email', 'email', _emailMsg, _emailColor, _emailValid, _validateEmail, 145),
                            const SizedBox(height: 10),
                            _buildMainField(_passwordController, 'Change your password', 'pass', _passMsg, _passColor, _passValid, _validatePass, 145, isPass: true),
                            const SizedBox(height: 30),
  ...
  Widget _buildMainField(TextEditingController ctrl, String h, String id, String m, Color c, bool v, Function(String) onCh, double w, {bool isPass = false}) {
    ...
    return Column(children: [
      Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            isDense: true,
            prefixIcon: isPass ? const Opacity(opacity: 0, child: Icon(Icons.visibility, size: 20)) : null,
      ...
      if (active && m.isNotEmpty) _buildValidationRow(m, c, w, v),
    ]);
  }
  ...
  Widget _buildPlatformBlock(String p) {
    ...
    return Column(children: [
      Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [
        Container(width: 101, height: 44, alignment: Alignment.center,
            decoration: BoxDecoration(color: const Color(0xFF181826), borderRadius: BorderRadius.circular(12)),
    ...
    return Padding(padding: const EdgeInsets.only(top: 4, bottom: 8),
        child: Row(children: [
          Expanded(child: Center(child: SizedBox(width: w,
              child: Text(m, textAlign: TextAlign.center, style: TextStyle(fontFamily: 'Poppins', fontSize: 10, color: c, height: 1.2))))),
          Container(width: 156, height: 44,
              decoration: BoxDecoration(color: const Color(0xFF181826), borderRadius: BorderRadius.circular(12), border: v ? Border.all(color: const Color(0xFF00F5A0)) : null),
              child: const Center(child: Text('Save', style: TextStyle(color: Colors.white54, fontSize: 14)))),
        ]));
  }
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          ...
          onPressed: _isFormReady ? _applyChanges : null,
          child: const Text(
            'APPLY CHANGES',
            style: TextStyle(
