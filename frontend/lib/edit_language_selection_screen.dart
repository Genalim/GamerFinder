...
class EditLanguageSelectionScreen extends StatefulWidget {
  const EditLanguageSelectionScreen({super.key});

  @override
  State<EditLanguageSelectionScreen> createState() => _EditLanguageSelectionScreenState();
}

class _EditLanguageSelectionScreenState extends State<EditLanguageSelectionScreen> {
  ...
  void initState() {
    super.initState();
    // Підтягуємо мови з профілю сесії
    final currentLanguages = UserSession().currentUser?.languages ?? [];
    _selectedLanguages = Set.from(currentLanguages);
  }
  ...
  Future<void> _applyChanges() async {
    try {
      ...
      final response = await http.put(
        Uri.parse("${ApiConfig.baseUrl}/users/$userId/languages"),
        headers: {
          "Content-Type": "application/json",
          if (token != null) "Authorization": "Bearer $token",
        },
        body: json.encode(_selectedLanguages.toList()),
      );
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Не вдалося оновити мови")),
        );
      }
    } catch (e) {
      debugPrint("Помилка збереження мов: $e");
    }
  }
  ...
  Widget _buildApplyButton() {
    bool canApply = _selectedLanguages.isNotEmpty;
    return Padding(
